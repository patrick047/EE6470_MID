...
void Testbench::fetch_result() {
  ...
  sc_uint<8> grey_r, grey_g, grey_b;
#ifndef NATIVE_SYSTEMC
	i_greyr.reset();
  i_greyg.reset();
  i_greyb.reset();
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      grey_b = i_greyb.read();
        ...
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = grey_r;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = grey_g;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = grey_b;
    }
  }
